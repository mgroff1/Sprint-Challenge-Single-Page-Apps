...
> Answer: Client side routing is as it sounds, all apparent page navigation takes place on the Client side, no need to route requests through a server. All routing is typically handled through a Javascript Framework that gives the illusion of page navigation when all thats happening is re-rendering with different material.
